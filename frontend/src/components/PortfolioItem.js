  ...
  return (
    <Table.Row>
      ...
      <Table.Cell size="tiny">
        <Header as="h6" color={color}>
          $
          {props.transaction.pricing && props.transaction.ohlc
            ? (
                props.transaction.pricing.askPrice * props.transaction.quantity
              ).toFixed(2)
            : props.transaction.ohlc
            ? (
                props.transaction.ohlc.close.price * props.transaction.quantity
              ).toFixed(2)
            : "NA"}
        </Header>
      </Table.Cell>
